...
class InstrumentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create instrument' do
    assert_difference('Instrument.count') do
      post instruments_url,
           params: { instrument: { band: @instrument.band, condition: @instrument.condition, description: @instrument.description,
                                   finish: @instrument.finish, model: @instrument.model, price: @instrument.price, title: @instrument.title } }
    end
  ...
  test 'should update instrument' do
    patch instrument_url(@instrument),
          params: { instrument: { band: @instrument.band, condition: @instrument.condition, description: @instrument.description,
                                  finish: @instrument.finish, model: @instrument.model, price: @instrument.price, title: @instrument.title } }
